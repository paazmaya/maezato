 ...
 */
const getReposGraphQL = (options, list = [], pagination = false) => {

  if (options.verbose) {
    console.log(`Fetching information about all the user repositories for "${options.username}"`);
  }
  options.query = pagination ?
    literals.QUERY_WITH_PAGINATION :
    literals.QUERY_WITHOUT_PAGINATION;
  ...
  return performRequest(options).then((response) => {
    ...
    return getReposGraphQL(options, list, true);
  }).catch((err) => Promise.reject(err));
};


module.exports = getReposGraphQL;
