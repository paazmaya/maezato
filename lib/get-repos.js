...
const performRequest = (options) => {
  ...
  return graphql({
    ...
    //require('fs').writeFileSync(`users-${options.username}-repos.json`, JSON.stringify(response, null, '  '), 'utf8');
 ...
 * @param {boolean} list[i].isArchived    If the repository is archived
