...
export const handlers = [
  graphql.operation(({request, operationName}) => {
    //console.log('[MSW] Request', request);
    //console.log(`[MSW] Intercepted a ${operationName} operation`);
    const authHeader = request.headers.get('authorization');

    const { ssh_url } = request.body;
    if (ssh_url === 'git@github.com:user/repo.git') {
      return HttpResponse.json({ message: 'Clone successful' }, { status: 200 });
    }

    if (ssh_url === 'git@github.com:user/clone-error.git') {
      return HttpResponse.json({ error: 'Repository already exists and is not empty' }, { status: 400 });
    }
